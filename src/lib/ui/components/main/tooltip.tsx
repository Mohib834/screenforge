...
function TooltipContent({
  ...
  return (
    <TooltipContentPrimitive
      className={cn(
        'z-50 w-fit rounded-lg border border-white/10 bg-[#1c1c1f] text-white shadow-xl',
        className,
      )}
      {...props}
    >
      <motion.div className="overflow-hidden px-2.5 py-1.5 text-xs font-medium tracking-wide text-balance">
        <motion.div layout={layout}>{children}</motion.div>
      </motion.div>
      <TooltipArrowPrimitive
        className="fill-[#1c1c1f] size-3 data-[side='bottom']:translate-y-[1px] data-[side='right']:translate-x-[1px] data-[side='left']:translate-x-[-1px] data-[side='top']:translate-y-[-1px]"
        tipRadius={2}
      />
    </TooltipContentPrimitive>
  );
